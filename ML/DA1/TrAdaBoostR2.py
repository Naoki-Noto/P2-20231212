...
data_s = pd.read_csv('data/data_s_{}.csv'.format(dataset))
data_s = data_s.drop(columns=['Name', 'ID'])
data_t = pd.read_csv('data/data_{}.csv'.format(dataset2))
...
Xs = data_s.drop(columns=['Yield',
                          #'HOMO', 'LUMO', 'E_S1', 'f_S1', 'E_T1', 'dDM',
                          #'P(dDM) * P(E_S1)', 'P(dDM) * P(E_T1)', 'P(dDM) * P(HOMO)', 'P(dDM) + P(HOMO)',
                          #'P(E_T1) - P(f_S1)', 'P(E_S1) / P(E_T1)', 'P(E_T1) / P(f_S1)',
                          ])
print(Xs)
...
for i in range(0,100):
    ...
    #print(Xt_lab)
    model = TrAdaBoostR2(hgb(), n_estimators=ne, Xt=Xt_lab, yt=yt_lab, random_state=0)
    model.fit(Xs, ys)
...
result.to_csv('result/result_unlab{}_{}_{}_ne{}.csv'.format(size, dataset, dataset2, ne))
