...
Xs = data_s.drop(columns=['Yield',
                          #'HOMO', 'E_S1', 'f_S1', 'E_T1', 'dEST', 'dDM',
                          #'P(dDM) * P(E_S1)', 'P(dEST) * P(f_S1)', 'P(E_S1) + P(f_S1)',
                          #'P(f_S1) - P(HOMO)', 'P(HOMO) / P(f_S1)', 'P(dDM) / P(E_S1)',
                          ])
#print(Xs)
...
#print(Xt_lab)
model = TrAdaBoostR2(hgb(), n_estimators=29, Xt=Xt_lab, yt=yt_lab, random_state=0, verbose=0)
model.fit(Xs, ys)
