...
data_t_lab, data_t_unlab = train_test_split(data_t, test_size=size, random_state=89)
...
Xs = data_s.drop(columns=['Yield',
                          #'HOMO', 'LUMO', 'f_S1', 'E_T1', 'dDM',
                          #'P(E_T1) * P(LUMO)', 'P(dDM) * P(E_T1)', 'P(dDM) + P(LUMO)',
                          #'P(dDM) - P(LUMO)', 'P(f_S1) - P(HOMO)', 'P(LUMO) / P(dDM)',
                          ])
#print(Xs)
...
Xt_lab, Xt_unlab, yt_lab, yt_unlab = train_test_split(Xt, yt, test_size=size, random_state=89)
#print(Xt_lab)
model = TrAdaBoostR2(hgb(), n_estimators=31, Xt=Xt_lab, yt=yt_lab, random_state=0, verbose=0)
model.fit(Xs, ys)
...
plt.show()
fig.savefig("result/TrAB_{}.pdf".format(size))
