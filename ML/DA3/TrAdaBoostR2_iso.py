...
data_s = pd.read_csv('data/data_s_{}.csv'.format(source))
...
model = TrAdaBoostR2(hgb(), n_estimators=30, Xt=Xt_lab, yt=yt_lab, random_state=0, verbose=0)
model.fit(Xs, ys)
...
print(prediction)
prediction.to_csv('result/prediction_iso_{}.csv'.format(source))
