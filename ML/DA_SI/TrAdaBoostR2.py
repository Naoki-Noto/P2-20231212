...
data_s = pd.read_csv('data/data_s_S1_{}.csv'.format(descriptor))
data_s = data_s.drop(columns=['Name', 'ID'])
data_t = pd.read_csv('data/data_t_{}.csv'.format(descriptor))
...
for i in range(0,100):
    ...
    #print(Xt_lab)
    model = TrAdaBoostR2(hgb(), n_estimators=5, Xt=Xt_lab, yt=yt_lab, random_state=0)
    model.fit(Xs, ys)
...
result.to_csv('result/result_{}_{}.csv'.format(estimator, descriptor))
