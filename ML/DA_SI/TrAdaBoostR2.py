...
#descriptor = 'MK'
descriptor = 'mordred'
#descriptor = 'RDKit_pca'
#descriptor = 'MF_pca'
#descriptor = 'MK_pca'
#descriptor = 'mordred_pca'

### Choose estimators for naming files ###
#estimator, es = 'ab', ab
#estimator, es = 'rf', rf
#estimator, es = 'hgb', hgb
estimator, es = 'lgbm', lgbm
#estimator, es = 'xgb', xgb

data_s = pd.read_csv(f'data/data_s_S1_{descriptor}.csv')
data_s = data_s.drop(columns=['Name', 'ID'])
data_t = pd.read_csv(f'data/data_t_{descriptor}.csv')
...
for i in range(0,100):
    ...
    #print(Xt_lab)
    model = TrAdaBoostR2(es(), n_estimators=5, Xt=Xt_lab, yt=yt_lab, random_state=0)
    model.fit(Xs, ys)
...
result.to_csv(f'result/result_{estimator}_{descriptor}.csv')
